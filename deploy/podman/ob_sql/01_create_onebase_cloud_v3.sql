...
-- 2. 默认先连接维护库 pg
-- 3. 会创建 onebase 用户、创建数据库 onebase_cloud_v3、并统一授予 public schema 下对象权限
...
WHERE NOT EXISTS (
  SELECT 1
  FROM pg_database
  WHERE datname = 'onebase_cloud_v3'
)
\gexec
...
GRANT ALL PRIVILEGES ON DATABASE onebase_cloud_v3 TO onebase;
GRANT CONNECT, TEMPORARY ON DATABASE onebase_cloud_v3 TO onebase;

\connect onebase_cloud_v3
