import { Divider, Empty, Form, Input, Layout, Select, Typography } from '@arco-design/web-react';
import { CONFIG_TYPES, LoadedPlugin, PluginComponent } from '@ob/plugin/sdk';
...
const StandardSetters: Record<string, React.FC<any>> = {
  ...
  [CONFIG_TYPES.NUMBER_INPUT]: ({ value, onChange, label }) => (
    <DynamicTextInputConfig onChange={onChange} value={value} item={{ name: label, key: 'mock' }} inputType="number" />
  ),
    ...
    if (key) {
      return (
        <Form.Item label={name || key} key={key || idx}>
          <Input value={config[key]} onChange={(v) => handleConfigChange(key, v)} placeholder={`Edit ${key}`} />
        </Form.Item>
      );
    }
    ...
    <Layout style={{ height: '100%', flexDirection: 'row' }}>
      <Layout.Content style={{ padding: 20, flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        >
          {Comp ? (
            <Form form={form}>
              <Comp {...compProps} value={value} onChange={handleValueChange} />
              {(() => {
                const linkMode = compProps?.ocrConfig?.recognitionMode === 'linked';
                const linkFieldKey = compProps?.ocrConfig?.linkConfig?.linkField;
                const rules = Array.isArray(compProps?.ocrConfig?.linkConfig?.rules)
                  ? compProps.ocrConfig.linkConfig.rules
                  : [];
        ...
        <Input.TextArea value={JSON.stringify(config, null, 2)} autoSize={{ minRows: 3, maxRows: 10 }} readOnly />
      </Layout.Content>

      <Layout.Sider
        width={350}
        style={{ background: '#fff', padding: 20, borderLeft: '1px solid #eee', overflow: 'auto' }}
      >
