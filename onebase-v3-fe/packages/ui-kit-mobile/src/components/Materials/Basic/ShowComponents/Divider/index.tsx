...
const XDivider = memo((props: XDividerConfig) => {
  ...
  const titleRender = () => {
    ...
    if (styleType === 'style7') {
      return (
        <div className={`decor-title-${styleType} title`} style={{ borderBottomColor: `${color}` }}>
    ...
    if (styleType === 'style8') {
      return (
        <div className={`decor-${styleType} title`} style={{ backgroundColor: hexToRgba(color, 0.2) }}>
          <span
    ...
    if (styleType === 'style9') {
      return (
        <div
          className={`decor-${styleType} title`}
          style={
            {
    ...
    if (styleType === 'style10') {
      return (
        <div className={`decor-${styleType} title`}>
  ...
  return (
    <div className={`formWrapperOBMobile dividerWrapperOBMobile`}>
      <div className='formDivider'>
        {titleRender()}
        {tooltip?.display && <span className='desc' style={{ color: `${descriptionColor}` }}>{tooltip.text}</span>}
      </div>
    </div>
  );
});
