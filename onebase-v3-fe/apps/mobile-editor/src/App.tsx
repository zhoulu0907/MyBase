...
const App: React.FC<AppProps & { instanceId: string }> = ({ instanceId, props }) => {
  ...
  const [dragProps, setDragProps] = useState<EditorProps & { drag: boolean }>({ drag: true } as EditorProps & {
    drag: boolean;
  });
  ...
  return (
    <>
      <Routes>
        <Route
          path="/onebase/:tenantId/editor/form_editor"
          element={<FormEditor instanceId={instanceId} props={propsData} />}
        />
        <Route
