...
public class TableMap<K, V> implements Map<K, V>, Iterable<Map.Entry<K, V>>, Serializable {
	...
	public List<V> getValues(K key) {
		return CollUtil.getAny(
				this.values,
				CollUtil.indexOfAll(this.keys, (ele) -> ObjUtil.equal(ele, key))
		);
	}
	...
	public List<K> getKeys(V value) {
		return CollUtil.getAny(
				this.keys,
                CollUtil.indexOfAll(this.values, (ele) -> ObjUtil.equal(ele, value))
		);
	}
	...
	public boolean remove(final Object key, final Object value) {
		...
		for (int i = 0; i < size(); i++) {
			if (ObjUtil.equal(key, keys.get(i)) && ObjUtil.equal(value, values.get(i))) {
				removeByIndex(i);
				removed = true;
				// 移除当前元素，下个元素前移
				i--;
	...
	public boolean replace(final K key, final V oldValue, final V newValue) {
		for (int i = 0; i < size(); i++) {
			if (ObjUtil.equal(key, keys.get(i)) && ObjUtil.equal(oldValue, values.get(i))) {
				values.set(i, newValue);
				return true;
			}
		}
	...
	public V replace(final K key, final V value) {
		V lastValue = null;
		for (int i = 0; i < size(); i++) {
			if (ObjUtil.equal(key, keys.get(i))) {
