...
import com.cmsr.onebase.framework.common.tools.core.annotation.AnnotationUtil;
import com.cmsr.onebase.framework.common.tools.core.annotation.PropIgnore;
...
public class PropDesc {
	...
	public Object getValue(Object bean, Type targetType, boolean ignoreError) {
		...
		try {
			...
			if (false == ignoreError) {
				throw new RuntimeException(StrUtil.format("Get value of [{}] error!", getFieldName()), e);
			}
		}
		...
		if (null != result && null != targetType) {
			...
			return Convert.convert(targetType, result, ignoreError);
		}
		return result;
	}
	...
	public boolean isWritable(boolean checkTransient) {
		// 检查是否有getter方法或是否为public修饰
		if (null == this.setter && false == isPublic(this.field)) {
			return false;
		}
    ...
    public static boolean isPublic(Field field) {
        return field != null && java.lang.reflect.Modifier.isPublic(field.getModifiers());
    }
	...
	public PropDesc setValue(Object bean, Object value, boolean ignoreNull, boolean ignoreError, boolean override) {
		...
		if (null != value) {
			...
			if (false == propClass.isInstance(value)) {
				value = Convert.convert(propClass, value, ignoreError);
			}
		}
		...
		if (null != value || false == ignoreNull) {
			try {
				...
				if (false == ignoreError) {
                    throw new RuntimeException(StrUtil.format("Set value of [{}] error!", getFieldName()), e);
				}
				// 忽略注入失败
			}
		}
