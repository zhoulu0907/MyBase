package com.cmsr.onebase.framework.common.tools.core.bean.copier;



import com.cmsr.onebase.framework.common.tools.core.bean.PropDesc;
...
import com.cmsr.onebase.framework.common.tools.core.convert.TypeConverter;
...
public class CopyOptions implements Serializable {
	...
	protected TypeConverter converter = (type, value) -> {
		...
		if (value instanceof IJSONTypeConverter) {
			return ((IJSONTypeConverter) value).toBean(ObjUtil.defaultIfNull(type, Object.class));
		}

		return Convert.convert(type, value, ignoreError);
	};
...
//	}
	...
	protected Object convertField(Type targetType, Object fieldValue) {
		if((targetType instanceof Class && Date.class.isAssignableFrom((Class<?>) targetType)) && null != this.formatIfDate){
            try {
                SimpleDateFormat sdf = new SimpleDateFormat(this.formatIfDate);
                if (fieldValue instanceof String) {
                    return sdf.parse((String) fieldValue);
                }
                ...
                throw new IllegalArgumentException("日期格式转换失败: value=" + fieldValue + ", format=" + this.formatIfDate, e);
            }
		}
