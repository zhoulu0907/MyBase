...
-- public.metadata_data_method_execution_log definition

-- Drop table

-- DROP TABLE public.metadata_data_method_execution_log;

CREATE TABLE public.metadata_data_method_execution_log (
	id int8 GENERATED BY DEFAULT AS IDENTITY( INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START 1 CACHE 1 NO CYCLE) NOT NULL, -- 主键ID
	method_id int8 NOT NULL, -- 关联的系统数据方法ID
	request_params jsonb NULL, -- 请求参数的JSON格式
	primary_keys jsonb NULL, -- 主键集合的JSON格式
	data_sources jsonb NULL, -- 涉及的数据源信息JSON格式
	duration_ms int4 NULL, -- 执行耗时(毫秒)
	status varchar(32) NULL, -- 执行状态：SUCCESS-成功，FAILED-失败
	error_msg text NULL, -- 错误信息
	deleted int8 DEFAULT 0 NOT NULL, -- 删除标记：0-未删除，其他值-已删除
	...
	lock_version int8 DEFAULT 0 NULL, -- 乐观锁版本号
	tenant_id int8 DEFAULT 1 NOT NULL, -- 租户ID
	CONSTRAINT metadata_data_method_execution_log_pkey PRIMARY KEY (id)
);
CREATE INDEX idx_metadata_data_method_execution_log_create_time ON public.metadata_data_method_execution_log USING btree (create_time);
CREATE INDEX idx_metadata_data_method_execution_log_method_id ON public.metadata_data_method_execution_log USING btree (method_id);
CREATE INDEX idx_metadata_data_method_execution_log_status ON public.metadata_data_method_execution_log USING btree (status);
COMMENT ON TABLE public.metadata_data_method_execution_log IS '数据方法执行日志表，记录输入参数、主键集合、数据源信息、耗时、状态与异常';

-- Column comments

COMMENT ON COLUMN public.metadata_data_method_execution_log.id IS '主键ID';
COMMENT ON COLUMN public.metadata_data_method_execution_log.method_id IS '关联的系统数据方法ID';
COMMENT ON COLUMN public.metadata_data_method_execution_log.request_params IS '请求参数的JSON格式';
COMMENT ON COLUMN public.metadata_data_method_execution_log.primary_keys IS '主键集合的JSON格式';
COMMENT ON COLUMN public.metadata_data_method_execution_log.data_sources IS '涉及的数据源信息JSON格式';
COMMENT ON COLUMN public.metadata_data_method_execution_log.duration_ms IS '执行耗时(毫秒)';
COMMENT ON COLUMN public.metadata_data_method_execution_log.status IS '执行状态：SUCCESS-成功，FAILED-失败';
COMMENT ON COLUMN public.metadata_data_method_execution_log.error_msg IS '错误信息';
COMMENT ON COLUMN public.metadata_data_method_execution_log.deleted IS '删除标记：0-未删除，其他值-已删除';
COMMENT ON COLUMN public.metadata_data_method_execution_log.creator IS '创建者';
COMMENT ON COLUMN public.metadata_data_method_execution_log.create_time IS '创建时间';
COMMENT ON COLUMN public.metadata_data_method_execution_log.updater IS '更新者';
COMMENT ON COLUMN public.metadata_data_method_execution_log.update_time IS '更新时间';
COMMENT ON COLUMN public.metadata_data_method_execution_log.lock_version IS '乐观锁版本号';
COMMENT ON COLUMN public.metadata_data_method_execution_log.tenant_id IS '租户ID';
