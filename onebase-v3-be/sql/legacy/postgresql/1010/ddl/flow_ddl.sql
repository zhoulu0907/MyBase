...
-- DROP TABLE public.flow_definition;
...
CREATE TABLE public.flow_definition (
	...
	del_flag bpchar(1) DEFAULT '0'::character varying NULL, -- 删除标志
...
COMMENT ON COLUMN public.flow_definition.model_value IS '设计器模型（CLASSICS经典模型 MIMIC仿钉钉模型）';
...
COMMENT ON COLUMN public.flow_definition.ext IS '扩展字段，预留给业务系统使用';
...
-- DROP TABLE public.flow_his_task;
...
CREATE TABLE public.flow_his_task (
	...
	cooperate_type int2 DEFAULT 0 NOT NULL, -- 协作方式(1审批 2转办 3委派 4会签 5票签 6加签 7减签)
	collaborator varchar(500) NULL, -- 协作人
	skip_type varchar(10) NULL, -- 流转类型（PASS通过 REJECT退回 NONE无动作）
	flow_status varchar(20) NOT NULL, -- 流程状态（0待提交 1审批中 2审批通过 4终止 5作废 6撤销 8已完成 9已退回 10失效 11拿回）
	form_custom bpchar(1) DEFAULT 'N'::character varying NULL, -- 审批表单是否自定义（Y是 N否）
	form_path varchar(100) NULL, -- 审批表单路径
	ext text NULL, -- 扩展字段，预留给业务系统使用
	message varchar(500) NULL, -- 审批意见
	variable text NULL, -- 任务变量
	create_time timestamp NULL, -- 任务开始时间
	update_time timestamp NULL, -- 审批完成时间
	del_flag bpchar(1) DEFAULT '0'::character varying NULL, -- 删除标志
	tenant_id int8 NULL, -- 租户id
...
COMMENT ON COLUMN public.flow_his_task.ext IS '扩展字段，预留给业务系统使用';
...
-- DROP TABLE public.flow_instance;
...
CREATE TABLE public.flow_instance (
	...
	create_by varchar(64) DEFAULT ''::character varying NULL, -- 创建者
	create_time timestamp NULL, -- 创建时间
	update_time timestamp NULL, -- 更新时间
	ext varchar(500) NULL, -- 扩展字段，预留给业务系统使用
	del_flag bpchar(1) DEFAULT '0'::character varying NULL, -- 删除标志
...
COMMENT ON COLUMN public.flow_node.permission_flag IS '权限标识（权限类型:权限标识，可以多个，用@@隔开)';
...
COMMENT ON COLUMN public.flow_node.ext IS '节点扩展属性';
COMMENT ON COLUMN public.flow_node.del_flag IS '删除标志';
COMMENT ON COLUMN public.flow_node.tenant_id IS '租户id';


-- public.flow_process definition

-- Drop table

-- DROP TABLE public.flow_process;
...
CREATE TABLE public.flow_process (
	...
	process_definition text NULL,
	enable_status int2 DEFAULT 0 NOT NULL,
	publish_status int2 DEFAULT 0 NOT NULL,
	trigger_type varchar(64) NOT NULL, -- 流程类型，如表达触发，定时触发，API触发等
	...
	CONSTRAINT pk_flow_process PRIMARY KEY (id)
);
...
-- public.flow_process_date_field definition

-- Drop table

-- DROP TABLE public.flow_process_date_field;
...
CREATE TABLE public.flow_process_date_field (
	...
	deleted int8 DEFAULT 0 NOT NULL,
	tenant_id int8 DEFAULT 0 NOT NULL,
	CONSTRAINT pk_flow_process_date_field PRIMARY KEY (id)
);


-- public.flow_process_entity definition

-- Drop table

-- DROP TABLE public.flow_process_entity;
...
CREATE TABLE public.flow_process_entity (
	...
	deleted int8 DEFAULT 0 NOT NULL,
	tenant_id int8 DEFAULT 0 NOT NULL,
	CONSTRAINT pk_flow_process_entity PRIMARY KEY (id),
	CONSTRAINT uk_flow_process_entity_unique UNIQUE (process_id, deleted)
);


-- public.flow_process_form definition

-- Drop table

-- DROP TABLE public.flow_process_form;
...
CREATE TABLE public.flow_process_form (
	...
	deleted int8 DEFAULT 0 NOT NULL,
	tenant_id int8 DEFAULT 0 NOT NULL,
	CONSTRAINT pk_flow_process_form PRIMARY KEY (id),
	CONSTRAINT uk_flow_process_form_unique UNIQUE (process_id, deleted)
);


-- public.flow_process_stat definition

-- Drop table

-- DROP TABLE public.flow_process_stat;

CREATE TABLE public.flow_process_stat (
	id int8 NOT NULL, -- 主键Id
	process_id int8 NOT NULL, -- 流程Id
	last_exec_time timestamp NULL, -- 上次执行时间
	exec_count int8 DEFAULT 0 NOT NULL, -- 执行次数
	lock_version int8 DEFAULT 0 NOT NULL,
	creator int8 DEFAULT 0 NOT NULL,
	create_time timestamp DEFAULT CURRENT_TIMESTAMP NOT NULL,
	updater int8 DEFAULT 0 NOT NULL,
	...
	deleted int8 DEFAULT 0 NOT NULL,
	tenant_id int8 DEFAULT 0 NOT NULL,
	CONSTRAINT pk_flow_process_stat PRIMARY KEY (id),
	CONSTRAINT uk_flow_process_stat UNIQUE (process_id, deleted)
);

-- Column comments

COMMENT ON COLUMN public.flow_process_stat.id IS '主键Id';
COMMENT ON COLUMN public.flow_process_stat.process_id IS '流程Id';
COMMENT ON COLUMN public.flow_process_stat.last_exec_time IS '上次执行时间';
COMMENT ON COLUMN public.flow_process_stat.exec_count IS '执行次数';


-- public.flow_process_time definition

-- Drop table

-- DROP TABLE public.flow_process_time;
...
CREATE TABLE public.flow_process_time (
	...
	deleted int8 DEFAULT 0 NOT NULL,
	tenant_id int8 DEFAULT 0 NOT NULL,
	CONSTRAINT pk_flow_process_time PRIMARY KEY (id),
	CONSTRAINT uk_flow_process_time_unique UNIQUE (process_id, deleted)
);
...
COMMENT ON TABLE public.flow_skip IS '节点跳转关联表';
...
-- DROP TABLE public.flow_user;

CREATE TABLE public.flow_user (
	id int8 NOT NULL, -- 主键id
	"type" bpchar(1) NOT NULL, -- 人员类型（1待办任务的审批人权限 2待办任务的转办人权限 3待办任务的委托人权限）
...
CREATE INDEX user_associated_idx ON public.flow_user USING btree (associated);
