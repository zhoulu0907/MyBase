...
/**
 * OCR 插件 API 测试 - Dev 模式
 * <p>
 * 测试 OCR 插件的三个识别接口在 Dev 模式下的连通性
 * </p>
 *
 * @author chengyuansen
 * @date 2026-01-14
...
@TestPropertySource(properties = {
        ...
        "onebase.plugin.dev-class-paths[0]=target/classes"
})
public class OcrApiDevModeTest {

    private static final Logger log = LoggerFactory.getLogger(OcrApiDevModeTest.class);
     ...
     */
    @Test
    @DisplayName("OCR插件 - 身份证识别接口 (Dev模式)")
    void testIdCardRecognition() {
        ...
        log.info("✓ OCR插件 - 身份证识别接口测试通过 (Dev模式)");
    }
     ...
     */
    @Test
    @DisplayName("OCR插件 - 港澳台通行证识别接口 (Dev模式)")
    void testExitentrypermitRecognition() {
        ...
        log.info("✓ OCR插件 - 港澳台通行证识别接口测试通过 (Dev模式)");
    }
     ...
     */
    @Test
    @DisplayName("OCR插件 - 护照识别接口 (Dev模式)")
    void testPassportRecognition() {
        ...
        log.info("✅ OCR插件 - 护照识别接口测试通过 (Dev模式)");
    }
    ...
    private File getResourceFile(String resourcePath) {
        try {
            ...
            throw new RuntimeException("无法加载资源文件: " + resourcePath, e);
        }
    }
}
