...
-- 2. 默认先连接维护库 pg
-- 3. 会创建 onebase 用户、创建数据库 onebase_dolphinscheduler、并统一授予 public schema 下对象权限
...
SELECT format(
  ...
  'onebase_dolphinscheduler'
)
WHERE NOT EXISTS (
  SELECT 1
  FROM pg_database
  WHERE datname = 'onebase_dolphinscheduler'
)
\gexec
...
GRANT ALL PRIVILEGES ON DATABASE onebase_dolphinscheduler TO onebase;
GRANT CONNECT, TEMPORARY ON DATABASE onebase_dolphinscheduler TO onebase;

\connect onebase_dolphinscheduler
