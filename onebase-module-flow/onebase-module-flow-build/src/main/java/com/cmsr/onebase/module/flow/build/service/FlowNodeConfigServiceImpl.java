...
public class FlowNodeConfigServiceImpl implements FlowNodeConfigService {
    ...
    @Override
    public List<ConnectorListVO> getAllConnectors() {
        List<FlowNodeConfigDO> dos = flowNodeConfigRepository.listAllConnectors();
        return dos.stream()
                .filter(doObj -> {
                    // level1 cannot be empty
                    if (StringUtils.isBlank(doObj.getLevel1Code())) {
                        log.warn("[ConnectorList] Connector skipped due to empty level1, nodeCode: {}", doObj.getNodeCode());
                        return false;
                    }
                    return true;
                })
                .map(doObj -> {
                    ConnectorListVO vo = new ConnectorListVO();
