 ...
 */
@Data
@Schema(description = "创建连接器动作请求")
public class CreateConnectorActionReqVO {

    @Schema(description = "动作名称", requiredMode = Schema.RequiredMode.REQUIRED, example = "获取用户信息")
    @NotBlank(message = "动作名称不能为空")
    @Size(max = 128, message = "动作名称不能超过128个字符")
    private String actionName;

    @Schema(description = "动作编码", requiredMode = Schema.RequiredMode.REQUIRED, example = "GET_USER")
    @NotBlank(message = "动作编码不能为空")
    @Size(max = 64, message = "动作编码不能超过64个字符")
