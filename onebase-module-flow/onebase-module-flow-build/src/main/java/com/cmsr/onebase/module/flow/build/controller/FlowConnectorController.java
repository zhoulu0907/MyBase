...
public class FlowConnectorController {
    ...
    // ==================== 环境配置接口（已废弃，请使用 FlowConnectorEnvController） ====================
    ...
    @Operation(summary = "获取启用环境",
              description = "获取连接器当前启用的环境完整信息，未设置则返回null")
    @Parameter(name = "id", description = "连接器实例ID", required = true, example = "1")
    @GetMapping("/{id}/get-enabled-env")
    @Deprecated
    public CommonResult<FlowConnectorEnvLiteVO> getEnabledEnv(@PathVariable("id") Long id) {
        FlowConnectorEnvLiteVO enabledEnv = connectorService.getEnabledEnv(id);
