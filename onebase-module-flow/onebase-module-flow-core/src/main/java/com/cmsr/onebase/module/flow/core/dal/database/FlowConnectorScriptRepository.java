...
public class FlowConnectorScriptRepository extends BaseAppRepository<FlowConnectorScriptMapper, FlowConnectorScriptDO> {
    ...
    public FlowConnectorScriptDO findByApplicationAndUuid(Long applicationId, Long actionId, String actionUuid) {
        QueryWrapper query = this.query().select(FLOW_CONNECTOR_SCRIPT.ALL_COLUMNS)
                .where(FLOW_CONNECTOR_SCRIPT.APPLICATION_ID.eq(applicationId))
                .and(FLOW_CONNECTOR_SCRIPT.SCRIPT_UUID.eq(actionUuid).or(FLOW_CONNECTOR_SCRIPT.ID.eq(actionId)));
