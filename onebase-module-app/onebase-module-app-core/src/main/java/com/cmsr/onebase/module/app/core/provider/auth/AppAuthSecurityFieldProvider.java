...
public class AppAuthSecurityFieldProvider {
    ...
    public List<FieldPermissionItem> findFields(Long applicationId, Set<String> roleUuids, String menuUuid) {
        if (CollectionUtils.isEmpty(roleUuids)) {
            return Collections.emptyList();
        }
        List<AppAuthFieldDO> fieldDOS = TenantManager.withoutTenantCondition(() -> ApplicationManager.withoutApplicationCondition(() ->
                appAuthFieldRepository.findByAppIdAndRoleIdsAndMenuId(applicationId, roleUuids, menuUuid)
        ));
