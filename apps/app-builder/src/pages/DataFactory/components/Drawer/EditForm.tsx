...
const NodeEditForm: React.FC<NodeEditFormProps> = ({
  ...
  const initialValues: FormValues = {
    ...
    systemFields: {
      creator: node.fields.find((field: EntityField) => field.id === "creator")
        ? true
        : false,
      updater: node.fields.find((field: EntityField) => field.id === "updater")
        ? true
        : false,
      createTime: node.fields.find(
        (field: EntityField) => field.id === "createTime",
      )
        ? true
        : false,
      updateTime: node.fields.find(
        (field: EntityField) => field.id === "updateTime",
      )
        ? true
        : false,
      dataOwner: node.fields.find(
        (field: EntityField) => field.id === "dataOwner",
      )
        ? true
        : false,
      dataDepartment: node.fields.find(
        (field: EntityField) => field.id === "dataDepartment",
      )
        ? true
        : false,
    },
