...
export const PropertiesEdit: React.FC<PropertiesEditProps> = (props) => {
  ...
  // 替换对象的key时，保持顺序
  const replaceKeyAtPosition = (obj: Record<string, any>, oldKey: string, newKey: string, newValue: any) => {
  ...
  const updateProperty = (propertyValue: JsonSchema, propertyKey: string, newPropertyKey?: string) => {
    if (newPropertyKey) {
  ...
  const updateNewProperty = (propertyValue: JsonSchema, propertyKey: string, newPropertyKey?: string) => {
    // const newValue = { ...value }
    if (newPropertyKey) {
