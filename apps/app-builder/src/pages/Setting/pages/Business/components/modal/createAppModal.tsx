...
export const CreateAppModal: React.FC<ICreateAppModal> = ({
  visible,
  dropdownList,
  tableData,
  onCloseAppModal,
  ...
  return (
    <Modal
      title={<div style={{ textAlign: 'left' }}>添加应用</div>}
      visible={visible}
      onCancel={handleCancel}
      onOk={handleSaveModal}
    >
      <Form
        form={createNewAppForm}
        initialValues={{
          appTime: {
            ...
            {filterOptions.map((option) => (
              <Select.Option key={option.id} value={option.id}>
                <Space align="center" size={12}>
                  <Avatar style={{ backgroundColor: option.iconColor,fontSize: '16px' }}>
                    {option.iconName ? (
                      <DynamicIcon
