...
// 创建页面编辑器 store 的工厂函数
const createPageEditorStore = () => create<PageEditorStore>((set) => ({
    components: [],
    setComponents: (components: any[]) => set(() => ({ components })),
    delComponents: (cp_id: string) => set((state) => {
    ...
    setPageComponentSchemas: (cp_id: string, config: EditConfig) => set((state) => {
        const newMap = new Map(state.pageComponentSchemas);
        newMap.set(cp_id, config);
        return { pageComponentSchemas: newMap };
    }),
    delPageComponentSchemas: (cp_id: string) => set((state) => {
        const newMap = new Map(state.pageComponentSchemas);
        newMap.delete(cp_id);
        return { pageComponentSchemas: newMap };
    }),
    ...
    setColComponentsMap: (cp_id: string, columnsOrUpdater: any[][] | ((prevColumns: any[][]) => any[][])) => set((state) => {
    ...
    delColComponentsMap: (cp_id: string) => set((state) => {
        const newMap = new Map(state.colComponentsMap.colComponents);
        newMap.delete(cp_id);
        return { colComponentsMap: { colComponents: newMap } };
    }),
