...
@RestController("buildSemanticDynamicDataController")
@RequestMapping("/build/metadata/{tableName}")
@Validated
@Tag(name = "动态数据(语义)-构建态", description = "基于RecordDTO的动态数据接口(构建态)")
/**
 * 动态数据(语义)控制器 - 构建态
 *
 ...
 * - 接收路由参数与请求体
 * - 生成/透传并回写 `X-Trace-Id`
 * - 委派业务到 {@link com.cmsr.onebase.module.metadata.build.semantic.service.SemanticDynamicDataService}
 *
