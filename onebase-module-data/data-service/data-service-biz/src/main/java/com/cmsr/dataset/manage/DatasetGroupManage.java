...
import com.cmsr.dataset.dao.ext.mapper.CoreDataSetExtMapper;
import com.cmsr.dataset.dao.ext.po.DataSetNodePO;
import com.cmsr.dataset.dto.DataSetNodeBO;
import com.cmsr.dataset.utils.DatasetUtils;
import com.cmsr.dataset.utils.FieldUtils;
...
public class DatasetGroupManage {
    ...
    public List<BusiLeafVO> getLeaf(BusiNodeRequest request) {
        ...
        queryWrapper.orderByDesc("create_time");
        List<DataSetNodePO> pos = coreDataSetExtMapper.query(queryWrapper);
        List<BusiLeafVO> blfs = new ArrayList<>();
        Iterator<DataSetNodePO> iterator = pos.iterator();
        BusiLeafVO busiLeafVO = null;
        while (iterator.hasNext()) {
            DataSetNodePO next = iterator.next();
            busiLeafVO = new BusiLeafVO();
            busiLeafVO.setId(next.getId());
            busiLeafVO.setName(next.getName());
            blfs.add(busiLeafVO);
        }
    ...
    public List<DatasetTableDTO> getDetails(List<Long> ids) {
        if (ObjectUtils.isEmpty(ids)) {
            DEException.throwException(Translator.get("i18n_table_id_can_not_empty"));
        }
        List<DatasetTableDTO> list = new ArrayList<>();
