...
import com.cmsr.onebase.module.metadata.runtime.semantic.dto.FieldSchemaDTO;
 ...
 */
@Component
public class DataPermissionChecker implements RuntimePermissionChecker {
     ...
     */
    public boolean supports(RecordDTO recordDTO) {
     ...
     * 从值模型构建数据行：字段名 -> 原始值
     */
    private Map<String,Object> buildDataRow(RecordDTO recordDTO) {
        Map<String,Object> row = new HashMap<>();
        Map<String, ValueDTO> data = recordDTO.getValue().getData();
     ...
     */
    private Map<Long,String> buildFieldIdToNameMap(RecordDTO recordDTO) {
        Map<Long,String> map = new HashMap<>();
        List<FieldSchemaDTO> fields = recordDTO.getEntity().getFields();
        if (fields == null) { return map; }
        for (FieldSchemaDTO f: fields) {
