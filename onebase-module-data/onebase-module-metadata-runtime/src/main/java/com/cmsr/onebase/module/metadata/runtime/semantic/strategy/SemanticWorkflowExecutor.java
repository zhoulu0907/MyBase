...
public class SemanticWorkflowExecutor {
    ...
    public void postExecute(SemanticRecordDTO recordDTO) {
        ...
        if (connectors != null) {
            for (SemanticRelationSchemaDTO c : connectors) {
                ...
                Map<String, Object> obj = recordDTO.getEntityValue().getConnectorRawObject(name);
                if (obj != null && !obj.isEmpty()) {
                    triggerConnector(recordDTO, c.getTargetEntityId(), op, obj, false);
                } else {
                    List<Map<String, Object>> list = recordDTO.getEntityValue().getConnectorRawList(name);
                    if (list != null) {
                        for (Map<String, Object> row : list) {
                            if (row != null && !row.isEmpty()) { triggerConnector(recordDTO, c.getTargetEntityId(), op, row, false); }
                        }
                    }
                }
            }
     ...
     */
    private void triggerConnector(SemanticRecordDTO recordDTO, Long entityId, MetadataDataMethodOpEnum op, Map<String, Object> rawData, boolean before) {
        var mapped = fieldNameIdMapper.convertNameToId(entityId, rawData);
        EntityTriggerReqDTO reqDTO = new EntityTriggerReqDTO();
        reqDTO.setTraceId(recordDTO.getRecordContext().getTraceId());
        reqDTO.setEntityId(entityId);
