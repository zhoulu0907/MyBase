...
public class DataMethodApiImpl implements DataMethodApi {
    ...
    public Integer deleteDataByCondition(@Valid DeleteDataReqDTO reqDTO) {
        ...
        if (reqDTO.getEntityId() == null) { throw new IllegalArgumentException("实体ID不能为空"); }
        if (reqDTO.getConditionDTO() == null || reqDTO.getConditionDTO().isEmpty()) { throw new IllegalArgumentException("删除操作必须提供条件，防止全表删除"); }
    ...
    public List<List<EntityFieldDataRespDTO>> insertData(@Valid InsertDataReqDTO reqDTO) {
        ...
        if (reqDTO.getEntityId() == null) { throw new IllegalArgumentException("实体ID不能为空"); }
        if (reqDTO.getData() == null || reqDTO.getData().isEmpty()) { throw new IllegalArgumentException("插入数据内容不能为空"); }
    Map<String,Object> dataByName = dataMethodAssembler.convertIdKeyMapToNameKeyMap(reqDTO.getEntityId(), reqDTO.getData());
        Map<String,Object> resultMap = metadataDataMethodCoreService.createData(reqDTO.getEntityId(), dataByName, null);
    QueryResult qr = dataMethodAssembler.buildQueryResultFromCoreResult(reqDTO.getEntityId(), resultMap);
    ...
    public List<List<EntityFieldDataRespDTO>> updateData(@Valid UpdateDataReqDTO reqDTO) {
        ...
        if (reqDTO.getEntityId() == null) { throw new IllegalArgumentException("实体ID不能为空"); }
        if (reqDTO.getConditionDTO() == null || reqDTO.getConditionDTO().isEmpty()) { throw new IllegalArgumentException("更新操作必须提供条件，防止全表更新"); }
        if (reqDTO.getData() == null || reqDTO.getData().isEmpty()) { throw new IllegalArgumentException("更新内容不能为空"); }
        QueryRequest queryRequest = new QueryRequest();
        queryRequest.setEntityId(reqDTO.getEntityId());
    queryRequest.setConditionGroups(dataMethodAssembler.convertConditionGroups(reqDTO.getConditionDTO()));
        queryRequest.setLimit(1000);
        QueryResult result = metadataQueryService.queryByConditions(queryRequest);
        if (result.getRowDataList() == null || result.getRowDataList().isEmpty()) { return new ArrayList<>(); }
    Map<String,Object> updateData = dataMethodAssembler.convertIdKeyMapToNameKeyMap(reqDTO.getEntityId(), reqDTO.getData());
        List<Map<String,Object>> updatedList = new ArrayList<>();
        for (RowData row : result.getRowDataList()) {
            Object idObj = dataMethodAssembler.tryParseId(row.getRowId());
            try {
                Map<String,Object> updated = metadataDataMethodCoreService.updateData(reqDTO.getEntityId(), idObj, updateData, null);
