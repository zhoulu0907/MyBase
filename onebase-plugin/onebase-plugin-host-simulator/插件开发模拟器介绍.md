...
- **动态配置**：使用`plugin-config.yml` 配置文件模拟动态配置。
- **配置热重载**：实时监控 `plugin-config.yml` 配置文件，修改后**立即生效**，无需重启。
...
**文件位置优先级**：
1. **外部文件 (推荐)**：
    - **IDE 模式**：`target/plugin-config.yml` (与 classes 目录同级)
    - **JAR 模式**：JAR 包同级目录下的 `plugin-config.yml`
2. **内部文件 (默认)**：`src/main/resources/plugin-config.yml` (不支持热加载)

**使用示例**：
在 JAR包 同目录下创建 `plugin-config.yml`：
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/plugin-config` | 查看所有配置 |
| GET | `/api/plugin-config/{pluginId}` | 查看插件配置 |
| PUT | `/api/plugin-config/{pluginId}` | 更新插件配置 |
| POST | `/api/plugin-config/reload` | 重载配置文件 |
