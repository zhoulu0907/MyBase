...
### 1. 🕹️ 支持多种运行模式
支持 DEV/STAGING/PROD 三种运行模式，完美覆盖开发、测试、生产全流程。

###  2. 🔄 支持插件全生命周期
统一插件的加载、启动、停止、卸载管理，同时支持静态（启动时）和动态（运行时）管理。
...
- **代码热重载**：实时监控 `dev-class-paths` 下的 `.class` 文件，支持插件中所有类（包括扩展点和非扩展点类）的热重载。
- **防抖设计**：智能合并短时间内的多次变更，避免重复重载。
- **精确更新**：最大程度保证运行时稳定性。

### 5. ⚙️ 支持动态配置服务
通过注入PluginConfigQueryService，可实现插件配置的动态化。
- **动态配置**：使用`plugin-context.yml` 配置文件模拟动态配置。
- **配置热重载**：实时监控 `plugin-context.yml` 配置文件，修改后**立即生效**，无需重启。
...
模拟器内置了插件配置模拟服务，让插件开发者可以测试动态配置获取逻辑。

### 1. 配置文件方式 & 热加载

模拟器支持加载外部配置文件，并支持**热加载**（修改即生效）。

**文件位置优先级**：
1. **外部文件 (推荐)**：
    - **IDE 模式**：`target/plugin-context.yml` (与 classes 目录同级)
    - **JAR 模式**：JAR 包同级目录下的 `plugin-context.yml`
2. **内部文件 (默认)**：`src/main/resources/plugin-context.yml` (不支持热加载)

**使用示例**：
在 JAR包 同目录下创建 `plugin-context.yml`：
...
// HttpHandler 插件中通过依赖注入获取
@RestController
@RequestMapping("/plugin/my-plugin/api")
public class MyApiController implements HttpHandler {
    ...
    public String test() {
        // 获取配置
        String provider = pluginConfigQueryService.getConfigValue("my-plugin", "1.0.0", "provider", "default");
        Map<String, Object> allConfig = pluginConfigQueryService.getConfig("my-plugin", "1.0.0");
        return "OK";
    }
}
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/plugin-context` | 查看所有配置 |
| GET | `/api/plugin-context/{pluginId}` | 查看插件配置 |
| PUT | `/api/plugin-context/{pluginId}` | 更新插件配置 |
| POST | `/api/plugin-context/reload` | 重载配置文件 |
