...
*   **自动扫描扩展点**：在编译阶段自动扫描实现了 `HttpHandler` 接口的类，并在jar包的 `META-INF/extensions.idx` 中生成索引，无需手动注册。
...
| **`scan-extensions`** | `process-classes` | **扫描扩展点实现**。<br>扫描编译后的 `.class` 文件，查找实现了 OneBase 扩展接口（`HttpHandler`）的类，并在 `META-INF/extensions.idx` 中生成索引。 |
