...
const XDeptSelect = memo(
  (
    props: XDeptSelectConfig & {
      runtime?: boolean;
      detailMode?: boolean;
      isMultiple: boolean;
      form?: any;
    ...
    return (
      ...
      >
        {status === STATUS_VALUES[STATUS_OPTIONS.READONLY] ? (
          <div className="readonlyText">{selectedParseDeptName}</div>
        ) : (
          <Cell className={styles.deptCell} onClick={() => setVisible(true)}>
            {selectedParseDeptName ? (
              <Ellipsis
                className={`${styles.selectValue} ${layout === 'vertical' ? styles.verticalLayout : ''}`}
                text={selectedParseDeptName}
                maxLine={1}
              />
            ) : (
            ...
            <PopupSwiper visible={visible} close={(e) => handleCancel(e)} direction={popupDirection}>
              <div className={`${styles.inputDeptSelectPopupContainer} ${editPreview ? styles.editPreview : ''}`}>
                <div className={styles.popupHeaderOBMobile}>
                  ...
                  <Button inline type="primary" size="mini" onClick={handleConfirm}>
                    确定
                  </Button>
                </div>
                ...
                <div className={styles.container}>
                  {loading && <LoadingComp />}
                  {!loading && renderData?.children?.length === 0 && <div className={styles.empty}>暂无数据</div>}
                  {!loading &&
                    renderData?.children?.length > 0 &&
                    renderData?.children.map((item: any) => (
                      <div key={`dept-${item.key}`} className={styles.item}>
