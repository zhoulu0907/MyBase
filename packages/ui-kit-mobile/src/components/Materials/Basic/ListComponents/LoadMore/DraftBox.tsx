...
import { Ellipsis, Button, NoticeBar, Dialog, Toast, Badge, LoadMore, Sticky, PopupSwiper } from '@arco-design/mobile-react';
import { AppEntityField, deleteDraft, deleteDraftTable, getDraftPage, getEntityFieldsWithChildren, menuSignal } from '@onebase/app';
...
export default function DraftBox(props: IProps) {
  ...
  const handleGetDrafts = async () => {
    try {
      ...
      const newTableData = await Promise.all((cloneList || []).map(async (item: any) => {
        const newItem = item;
  ...
  return (
    <>
      <div className='draft' onClick={() => { setVisible(true) }}>
