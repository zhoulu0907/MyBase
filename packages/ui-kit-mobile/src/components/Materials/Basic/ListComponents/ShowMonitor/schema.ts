...
export interface XShowMonitorSchema {
  ...
  config: XShowMonitorConfig;
}

export type TXShowMonitorEditData = Array<
  | ITextConfigType
  | ILabelConfigType
...
export interface XShowMonitorConfig extends ICommonBaseType {
  /**
   * 输入框标题
   * text：标题
   * display：是否显示
...
const XShowMonitor: XShowMonitorSchema = {
  editData: [
    ...baseConfig,
    {
      key: 'label',
