...
import { type RoleVO, type RoleForm, type UpdateStatusForm } from '../types/role';
...
export const removeRolePermission = async (roleId: number, permissionId: number) => {
  return await systemClient.post('/role/remove-permission', { roleId, permissionId })
}

// 为某角色配置权限
export const configureRolePermissions = async (roleId: number, permissions: Record<number, number[]>) => {
  return await systemClient.post('/role/configure-permissions', { roleId, permissions })
}

// 获取所有权限
export const getAllPermissions = async (params: Record<string, any> = {}): Promise<Permission[]> => {
  return await systemClient.get('/permission/all', { params })
}

// 获取角色下已配置权限
export const getConfiguredPermissions = async (roleId: number): Promise<any> => {
  return await systemClient.get('/permission/configured', { params: { roleId } })
}
