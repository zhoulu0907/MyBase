...
@Slf4j
@Validated
public abstract class AbstractDeptServiceImpl implements DeptService {
     ...
     * 动态设置对应部门数据仓库
     */
    public abstract AbstractDeptDataRepository getDeptDataRepository();
    ...
    public Long createDept(DeptSaveReqVO createReqVO) {
        ...
        if (XFromSceneTypeEnum.TENANT.getCode().equals(fromSceneType)) {
            ...
            throw exception(DEPT_TYPE_EXCEPTION, fromSceneType);
        }
    ...
    private long getChildDeptCount(Long parentId) {
        return getDeptDataRepository().findAllByParentId(parentId).size();
    }
    ...
    void validateDeptExists(Long id) {
        ...
        DeptDO dept = getDeptDataRepository().findById(id);
        if (dept == null) {
    ...
    void validateParentDept(Long id, Long parentId) {
        ...
        // 2. 父部门不存在
        DeptDO parentDept = getDeptDataRepository().findById(parentId);
        if (parentDept == null) {
        ...
        for (int i = 0; i < Short.MAX_VALUE; i++) {
            ...
            parentDept = getDeptDataRepository().findById(parentId);
            if (parentDept == null) {
                break;
            }
        }
    ...
    void validateDeptNameUnique(Long id, Long parentId, String name) {
        DeptDO dept = getDeptDataRepository().findOneByParentIdAndName(parentId, name);
        if (dept == null) {
            return;
        }
    ...
    public DeptDO getDept(Long id) {
        ...
        return getDeptDataRepository().findById(id);
    }
    ...
    public List<DeptDO> getDeptList(Collection<Long> ids) {
        ...
        return getDeptDataRepository().findAllByIds(new ArrayList<>(ids));
    }

    @Override
    public List<DeptDO> getDeptList(DeptListReqVO reqVO) {
        return getDeptDataRepository().findAllByNameAndStatus(reqVO.getName(), reqVO.getStatus());
    }
    ...
    private List<DeptDO> getDeptListByParentIds(Collection<Long> parentIds) {
        return getDeptDataRepository().findAllByParentIds(parentIds);
    }

    @Override
    public List<DeptDO> getDeptListByLeaderUserId(Long id) {
        return getDeptDataRepository().findAllByLeaderUserId(id);
    }
    ...
    public DeptAndUsersRespVO getDeptAndUsers(DeptAndUsersReqVO reqVO) {
        ...
        if (hasKeywords) {
            ...
            // 按部门名称模糊搜索部门
            List<DeptDO> matchedDepts = getDeptDataRepository().findAllByNameAndStatus(reqVO.getKeywords(), null);
            ...
            // 获取直属子部门
            List<DeptDO> childDepts = getDeptDataRepository().findAllByParentId(reqVO.getDeptId());
            ...
            // 获取所有一级部门（parentId = 0）
            List<DeptDO> rootDepts = getDeptDataRepository().findAllByParentId(DeptDO.PARENT_ID_ROOT);
    ...
    private List<DeptDO> getParentDeptsList(Long deptId) {
        ...
        DeptDO dept = getDeptDataRepository().findById(deptId);
        parentDepts.add(dept);

        int loopCount = LOOP_INIT;
        while (dept != null && dept.getParentId() != null) {
            DeptDO parentDept = getDeptDataRepository().findById(dept.getParentId());
            if (parentDept != null) {
    ...
    public void updateAdminOrDirector(UserAdminOrDirectorUpdateReqVO reqVO) {
        ...
        if (reqVO.getUpdateType().equals(CorpConstant.LEADER_USER_ID)) {
            ...
            getDeptDataRepository().updateByConfig(row, new DefaultConfigStore().eq(DeptDO.ID, reqVO.getDeptId()));
        } else {
            DataRow row = new DataRow();
