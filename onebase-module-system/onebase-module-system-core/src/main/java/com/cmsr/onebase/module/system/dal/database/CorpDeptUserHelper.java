...
public class CorpDeptUserHelper {
    ...
    public static DefaultConfigStore getCorpConfigStore(String entityType) {
        ...
        if (XFromSceneTypeEnum.TENANT.getCode().equals(fromSceneType)) {
            configStore.and(Compare.EQUAL, entityType, DeptTypeEnum.TENANT.getCode());
            ...
            if (null != corpId) {
                ...
                configStore.and(Compare.EQUAL, entityType, DeptTypeEnum.CORP.getCode());
            } else {
                // 立即失败，抛出异常，防止数据越权
