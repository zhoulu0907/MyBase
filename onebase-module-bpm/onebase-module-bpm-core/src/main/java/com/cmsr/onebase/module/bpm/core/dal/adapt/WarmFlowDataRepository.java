...
import com.cmsr.onebase.module.bpm.core.warmflow.anyline.dataobject.BpmWarmFlowBaseDO;
 ...
 */
@Slf4j
public class WarmFlowDataRepository<T extends BpmWarmFlowBaseDO> {
    ...
    public WarmFlowDataRepository(Class<T> defaultClazz) {
     ...
     * @return 表名
     */
    private String getTableName(Class<? extends BpmWarmFlowBaseDO> clazz) {
    ...
    public long update(T entity) {
        ...
        try {
            ...
            configs.eq(BpmWarmFlowBaseDO.ID, entity.getId());
    ...
    public T findById(Long id) {
        try {
            ...
            ConfigStore configs = new DefaultConfigStore();
            configs.and(Compare.EQUAL, BpmWarmFlowBaseDO.ID, id);
    ...
    public List<T> findAllByIds(Collection<Long> ids) {
        ...
        try {
            ConfigStore configs = new DefaultConfigStore();
            configs.in(BpmWarmFlowBaseDO.ID, ids);
            String tableName = getTableName(defaultClazz);
            DataSet dataSet = anylineService.querys(tableName, configs);
            return dataSet.entity(defaultClazz);
        } catch (Exception e) {
    ...
    public long deleteByConfig(ConfigStore configs) {
        try {
            ...
            row.put(BpmWarmFlowBaseDO.DELETED, System.currentTimeMillis());  // 设置逻辑删除标记
    ...
    public long deleteByIds(List<Long> ids) {
        try {
            ...
            configs.in(BpmWarmFlowBaseDO.ID, ids);
            DataRow row = new DataRow();
            row.put(BpmWarmFlowBaseDO.DELETED, System.currentTimeMillis());
            long result = anylineService.update(getTableName(defaultClazz), row, configs);
    ...
    public long deleteAllById(Collection<Long> ids) {
        try {
            ...
            ConfigStore configs = new DefaultConfigStore();
            configs.and(Compare.IN, BpmWarmFlowBaseDO.ID, ids);
            DataRow row = new DataRow();
            row.put(BpmWarmFlowBaseDO.DELETED, System.currentTimeMillis());  // 设置逻辑删除标记
            long result = anylineService.update(getTableName(defaultClazz), row, configs);
